...
class Dish {
  constructor(dishData) {
    ...
    const restaurantId = dishData.restaurant || dishData.restaurantId
    this.restaurantId = new ObjectId(restaurantId)
    this.categoryId = dishData.category ? new ObjectId(dishData.category) : null
    this.ingredients = dishData.ingredients || []
    this.allergens = dishData.allergens || []
    this.nutritionalInfo = dishData.nutritionalInfo || {}
    this.preparationTime = dishData.preparationTime || null
    this.spicyLevel = dishData.spicyLevel || 0
    this.tags = dishData.tags || []
  ...
  static async create(dishData) {
    ...
    if (dishData.category) {
      const category = await db.collection("categories").findOne({
        _id: new ObjectId(dishData.category),
      })
      ...
      .aggregate([
        { $match: { _id: new ObjectId(id) } },
        {
          $lookup: {
            ...
            foreignField: "_id",
            as: "restaurant",
            pipeline: [{ $project: { name: 1, category: 1, location: 1 } }],
          },
        },
        {
          $addFields: {
  ...
  static async findByRestaurant(restaurantId, options = {}) {
    ...
    const [dishes, total] = await Promise.all([
      db.collection("dishes").find(query).sort(sort).skip(skip).limit(limit).toArray(),
    ...
    const pipeline = [
      { $match: query },
      {
        $lookup: {
          ...
          foreignField: "_id",
          as: "restaurant",
          pipeline: [{ $project: { name: 1, category: 1, approved: 1 } }],
        },
      },
      {
        $addFields: {
  ...
  static async getStats() {
    ...
    const dishesByRestaurant = await db
      .collection("dishes")
      .aggregate([
        {
          $lookup: {
