...
class Restaurant {
  constructor(restaurantData) {
    ...
    this.location = {
      ...
      coordinates: {
        type: 'Point',
        coordinates: restaurantData.location.coordinates.coordinates // [longitude, latitude]
      }
    };
    this.image = restaurantData.image || null;
    this.approved = restaurantData.approved || false;
    this.rating = 0;
    this.reviewCount = 0;
  ...
  static async findById(id) {
    ...
    if (restaurant) {
      ...
      const dishes = await db.collection('dishes').find({ restaurantId: new ObjectId(id) }).toArray();
  ...
  static async getStats() {
    ...
    const categoryStats = await db.collection('restaurants').aggregate([
      { $match: { approved: true } },
      {
        $group: {
          _id: '$category',
